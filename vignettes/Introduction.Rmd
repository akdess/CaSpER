...
loh.name.mapping:  data frame  contains the sample id mappings between expression signal and baf signal. For bulk sequencing data there will be one to one mapping since baf signal is generated for all samples seperately. 

```{r, eval=T}
kable(yale_meningioma$loh.name.mapping[1:5, ])
```

However, in single-cell sequencing data there will be one to multiple mapping since baf signal is generated after pooling all the cells coming from the same patient. control.sample.ids is the vector of control samples.
...
knitr::include_graphics("C:\\Users\\aharmanci\\Google Drive\\uthealth\\reports\\manuscripts\\casper_revision\\test.png")
```
...
``` 
```{r, echo=FALSE, out.width = '50%'}
knitr::include_graphics("C:\\Users\\aharmanci\\Google Drive\\uthealth\\reports\\manuscripts\\casper_revision\\large.scale.events.png")
```
...
``` 
```{r, echo=FALSE, out.width = '50%'}
knitr::include_graphics("C:\\Users\\aharmanci\\Google Drive\\uthealth\\reports\\manuscripts\\casper_revision\\RNASeqAndGT.png")
```

c. plotBAFAllSamples: Visualization of BAF shift signal for all samples together 
The inspection of BAF shift signal is useful especially when compared to the expression signal to analyze the CNV and LOH events. inferCNV does not provide any visualizations regarding the genomewide BAF shift signal and we believe that this visualization can enable a more comprehensive analysis of the above-mentioned events. The BAF shift plots show the BAF shift signal such that each row is the genomewide BAF shift signal profile. An example is included below for reference.
...
```{r, echo=FALSE, out.width = '50%'}
knitr::include_graphics("C:\\Users\\aharmanci\\Google Drive\\uthealth\\reports\\manuscripts\\casper_revision\\LOHAllSamples.png")
```
...
```{r, echo=FALSE, out.width = '50%'}
knitr::include_graphics("C:\\Users\\aharmanci\\Google Drive\\uthealth\\reports\\manuscripts\\casper_revision\\MN-60.png")
```

e. plotBAFInSeperatePages: Visualization of BAF deviation for each sample in separate pages. This option creates the BAF shift signal for each sample separately. This way the user can visualize each sample by itself. An example is shown below.
...
```{r, echo=FALSE, out.width = '50%'}
knitr::include_graphics("C:\\Users\\aharmanci\\Google Drive\\uthealth\\reports\\manuscripts\\casper_revision\\MN-5.png")
```

f. plotGEAndBAFOneSample: Gene expression and BAF signal for one sample in one plot. This option generates the visualization of the gene expression signal and BAF shift signal together so that the user can jointly assess them in one page. An example figure is included below. This plot shows the original data and the smoothed data for all the scales to include a complete visualization of the multiscale decomposition.
...
```{r, echo=FALSE,  out.width = '50%'}
knitr::include_graphics("C:\\Users\\aharmanci\\Google Drive\\uthealth\\reports\\manuscripts\\casper_revision\\MN-5_GE_BAF.png")
```
...
knitr::include_graphics("C:\\Users\\aharmanci\\Google Drive\\uthealth\\reports\\manuscripts\\casper_revision\\mgh31_ls.png")
```
...
```
```{r, echo=FALSE,  out.width = '50%'}
knitr::include_graphics("C:\\Users\\aharmanci\\Google Drive\\uthealth\\reports\\manuscripts\\casper_revision\\MGH30network.png")
```
...
2. [TCGA-BRCA Bulk RNA-Seq dataset](/demo/tcga_BRCA.R)
